# -*- coding: utf-8 -*-
"""
Parser cho Bảng kê TA035 (Bán ra - TK33311).
"""

import pandas as pd
import pdfplumber
import re
from .utils import clean_number, clean_text, clean_status


def parse_ta035(file_or_path):
    """
    Parse file PDF Bảng kê TA035 (bán ra).
    
    Returns:
        dict: {
            'data': DataFrame chi tiết,
            'by_tax_group': DataFrame tổng hợp theo nhóm thuế suất,
            'by_product': DataFrame tổng hợp theo mã sản phẩm,
            'totals': dict tổng cộng
        }
    """
    if isinstance(file_or_path, str):
        pdf = pdfplumber.open(file_or_path)
    else:
        pdf = pdfplumber.open(file_or_path)
    
    all_rows = []
    current_group = ""
    group_map = {
        "1": "Không chịu thuế GTGT",
        "2": "Thuế suất 0%",
        "3": "Thuế suất 5%",
        "4": "Thuế suất 10%",  # Thực tế là 8% theo NĐ giảm thuế
        "5": "Không tổng hợp trên tờ khai",
    }
    
    for page in pdf.pages:
        tables = page.extract_tables()
        if not tables:
            continue
        
        for table in tables:
            for row in table:
                if not row or len(row) < 10:
                    continue
                
                first_cell = clean_text(row[0]) if row[0] else ""
                
                # Detect nhóm thuế suất
                for key, value in group_map.items():
                    pattern = f"{key}."
                    group_pattern = f"{key}.Hàng hóa"
                    full_text = clean_text(" ".join([str(c) for c in row if c]))
                    if first_cell.startswith(pattern) and ("Hàng hóa" in full_text or "hàng hóa" in full_text or "Có số liệu" in full_text):
                        current_group = value
                        break
                
                # Skip headers and special rows
                if first_cell in ["STT", "ST\nT", "[1]", "1", ""] and (
                    not row[1] or clean_text(row[1]) in ["2", "Ký hiệu", ""]):
                    if first_cell == "[1]" or (row[1] and clean_text(row[1]) == "[2]"):
                        continue
                    if first_cell == "" and (not row[1] or clean_text(row[1]) == ""):
                        continue
                
                # Dòng tổng cộng
                if "Tổng cộng" in first_cell or "Tổng doanh thu" in first_cell:
                    continue
                
                # Check nếu là STT hợp lệ
                stt_val = first_cell.strip()
                try:
                    stt_num = int(stt_val)
                except (ValueError, TypeError):
                    continue
                
                # Parse dữ liệu
                try:
                    mat_hang = clean_text(row[6]) if len(row) > 6 else ""
                    doanh_so = clean_number(row[7]) if len(row) > 7 else 0
                    thue_suat = clean_number(row[8]) if len(row) > 8 else 0
                    thue_gtgt = clean_number(row[9]) if len(row) > 9 else 0
                    trang_thai = clean_status(row[18]) if len(row) > 18 else ""
                    
                    # Skip dòng trống (mẫu empty)
                    if doanh_so == 0 and thue_gtgt == 0 and not mat_hang:
                        continue
                    
                    data_row = {
                        "stt": stt_num,
                        "nhom_thue": current_group,
                        "ky_hieu_hd": clean_text(row[1]) if len(row) > 1 else "",
                        "so_hoa_don": clean_text(row[2]) if len(row) > 2 else "",
                        "ngay_phat_hanh": clean_text(row[3]) if len(row) > 3 else "",
                        "ten_nguoi_mua": clean_text(row[4]) if len(row) > 4 else "",
                        "ma_so_thue": clean_text(row[5]) if len(row) > 5 else "",
                        "mat_hang": mat_hang,
                        "doanh_so": doanh_so,
                        "thue_suat": thue_suat,
                        "thue_gtgt": thue_gtgt,
                        "ghi_chu": clean_text(row[10]) if len(row) > 10 else "",
                        "nguon_ct": clean_text(row[11]) if len(row) > 11 else "",
                        "so_ct": clean_text(row[12]) if len(row) > 12 else "",
                        "ngay_lap_ct": clean_text(row[13]) if len(row) > 13 else "",
                        "so_tien_da_tt": clean_number(row[15]) if len(row) > 15 else 0,
                        "nguoi_lap_ct": clean_text(row[17]) if len(row) > 17 else "",
                        "trang_thai": trang_thai,
                    }
                    all_rows.append(data_row)
                except (IndexError, ValueError):
                    continue
    
    pdf.close()
    
    if not all_rows:
        return {
            "data": pd.DataFrame(),
            "by_tax_group": pd.DataFrame(),
            "by_product": pd.DataFrame(),
            "totals": {},
        }
    
    df = pd.DataFrame(all_rows)
    
    # Tổng hợp theo nhóm thuế
    by_tax_group = df.groupby("nhom_thue").agg({
        "doanh_so": "sum",
        "thue_gtgt": "sum",
        "stt": "count",
    }).rename(columns={"stt": "so_luong"}).reset_index()
    
    # Tổng hợp theo mã hàng
    by_product = df.groupby("mat_hang").agg({
        "doanh_so": "sum",
        "thue_gtgt": "sum",
        "thue_suat": "first",
        "stt": "count",
    }).rename(columns={"stt": "so_luong"}).reset_index()
    
    totals = {
        "tong_doanh_so": df["doanh_so"].sum(),
        "tong_thue_gtgt": df["thue_gtgt"].sum(),
    }
    
    return {
        "data": df,
        "by_tax_group": by_tax_group,
        "by_product": by_product,
        "totals": totals,
    }
